...
- **Recommended addition, scoped to be feasible, not a new architecture:** add at least one downstream-task comparison (SR output vs. bicubic-upsampled LR vs., where available, real HR) using an existing label source aligned to the same AOIs already in the plan.
- **Verified this round (round 6):** GeoSR-Bench's "check before assuming" hedge from round 5 is resolved — its two cross-platform SR tasks are MODIS→Landsat-8 (500m→30m) *and specifically Sentinel-2→NAIP (10m→0.6m)*, the same sensor pair as SEN2NAIP, already this project's primary training dataset. That's a better match than round 5 knew: pixel-level downstream labels (land cover segmentation, infrastructure mapping, biophysical variable estimation) on the exact SR task this project is doing, plus built-in comparison against 9 other SR models already benchmarked on it (GAN/transformer/neural-operator/diffusion). Source: arxiv.org/abs/2605.00310.
- **But — practical catch, checked before recommending it as primary:** the public release (huggingface.co/datasets/ai-spatial/GeoSR-Bench) is marked "Dataset Upload in Progress" as of this check, with an active data-loading error on the dataset viewer. It's the better-fit resource on paper, but not yet reliably usable on a hackathon timeline. **Recommendation: use the ESA WorldCover route (10m, global, Sentinel-1/2-derived, free, includes India) as the primary/reliable downstream-task check now, and treat GeoSR-Bench as worth re-checking closer to build time** — if its release stabilizes, it's the stronger benchmark and worth switching to or adding. Report a downstream metric (e.g., IoU or per-class accuracy) alongside the existing image-quality metrics, not instead of them.
...
### External baselines (revised, round 3; extended round 8 — see Known Risks #7 and #11)
...
- **SEN2SR / SEN2SRLite (new, round 8)** — ESA OpenSR's newer, actively-maintained package (github.com/ESAOpenSR/SEN2SR, `pip install sen2sr`), super-resolving both 10m and 20m Sentinel-2 bands to 2.5m. Evaluates CNN, Mamba, and Swin Transformer architectures with a "low-frequency hard constraint layer" that enforces spectral consistency by construction — a concrete technique worth citing even if not adopted directly. Reports PSNR superior to compared SOTA with near-zero reflectance deviation. Trained/validated on SEN2NAIPv2 and OpenSR-Test — the same dataset family already in this project's plan. **SEN2SRLite is the lightweight variant, pip-installable, Colab-runnable, no Mamba dependency required** — i.e., a working, low-compute Sentinel-2 SR tool already exists and is trivial to run today. Source: sciencedirect.com/science/article/pii/S0034425725006261, github.com/ESAOpenSR/SEN2SR, pypi.org/project/sen2sr
...
**Round 8 update — this narrows the choice further, not just adds an option.** SEN2SRLite is a pip-installable, actively-maintained, low-compute Sentinel-2 SR tool from the *same* ESA team, publicly available today. Combined with the round 4 finding that at least one hackathon competitor has already executed a compute-tier comparison well, **path B (compute-tier alternative) is now effectively closed as a novelty claim** — not just "less differentiated," genuinely redundant with an official, easy-to-run package. Path C (comparative benchmark study) is also weaker than it looked: SEN2SR's own paper already includes a CNN/Mamba/Swin architecture comparison with explainability analysis, more rigorous than a hackathon team is likely to match. **Path A — real India-geography validation against actual Cartosat-2S/3 ground truth — is now the clear, essentially sole remaining differentiator.** Nothing found across 8 rounds does this. This should be treated as the team's committed direction, not one of three options, unless a future round finds a reason to reopen it.
...
1. **Architecture comparison (revised, round 3; extended round 8):** Not a settled "CNN beats diffusion" or vice versa. Generic RSISR diffusion models (EDiffSR etc., benchmarked on UCMerced/AID) do trade fidelity for perceptual realism. But domain-specialized, Sentinel-2-specific diffusion models (DiffFuSR, Donike et al. 2025) explicitly report *strong* fidelity and hallucination-suppression, not the generic tradeoff. SEN2SR adds a third data point: a CNN/Mamba/Swin comparison from the same ecosystem, with an explicit spectral-consistency-by-construction technique (low-frequency hard constraint layer) worth studying regardless of which backbone the team ends up using. The real constraint remains compute for the diffusion options (#7 below), not architecture quality in the abstract.
...
10. **Downstream-task utility (round 5, refined round 6):** the PS explicitly asks for improved "interpretability and analytical utility" and names classification, change detection, crop monitoring, urban mapping, and disaster response as target applications. Fidelity metrics alone don't reliably predict this (GeoSR-Bench, 2026) — sometimes the correlation is negative. Resolution: use ESA WorldCover for a reliable downstream check now; GeoSR-Bench is a better-matched benchmark (its Sentinel-2→NAIP task is exactly this project's setup) but its public release isn't stable yet — worth rechecking, not worth blocking on.
11. **Path B and C novelty are now effectively closed, round 8:** SEN2SRLite (ESA OpenSR, pip-installable, low-compute) plus the round-4 competitor finding mean a "lightweight alternative" or "we ran a comparison" story is redundant with existing, easy-to-access work. Path A (India/Cartosat ground-truth validation) is the only differentiator no round has found prior art or a competitor for. Recommend the team treat this as decided, not still-open, going into execution.
...
- SEN2SR / SEN2SRLite: https://github.com/ESAOpenSR/SEN2SR ; https://pypi.org/project/sen2sr/ ; https://www.sciencedirect.com/science/article/pii/S0034425725006261
